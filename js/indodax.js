...
module.exports = class indodax extends Exchange {
        ...
        return this.deepExtend (super.describe (), {
            ...
            'wsconf': {
                'conx-tpls': {
                    'default': {
                        'type': 'pusher',
                        'baseurl': 'wss://ws-ap1.pusher.com:443/app/a0dfa181b1248b929b11',
                    },
                },
                'methodmap': {
    ...
    findMarket (string) {
        ...
        if (typeof string === 'string') {
            for (let marketKey of Object.keys(this.markets_by_id)) {
                if (marketKey.replace('_', '') === string) {
                    return this.markets_by_id[marketKey]
                }
            }
    ...
    findSymbol (string, market = undefined) {
        ...
        if (typeof market !== 'object') {
            market = this.findMarket (string.replace('/', '_').toLowerCase())
        }
    ...
    _websocketHandleOrderbook (contextId, msg) {
        ...
        let ob = this.parseOrderBook (data, timestamp * 1000, 'buy_orders', 'sell_orders');
